...
description: In two months MiniMax M3 burned 16.89B tokens. That mix — plus Grok Heavy — shipped BRNN AI, Clinicsy, Mentoring, TibiaPixel and the rest of the Brenon.Cloud catalog through agentic loops, for a fraction of Claude or ChatGPT API rates.
date: 2026-08-28
...
Grok does not publish the same total-token counter. Add its weekly Heavy pool, OpenRouter credits, Gemini, and local models, and the whole operation lands around **~20B tokens**. Those tokens did not stay on a dashboard. They shipped products we still **build, publish, and operate** on Brenon.Cloud — BRNN AI, Clinicsy, DevDojo Mentoring, TibiaPixel, and the rest of the catalog — through end-to-end agentic loops.

The thesis is simple: you can run a fully agentic business operation by mixing cheap high-volume Chinese plans with expensive, competent American models where it matters — and the receipt is a different order of magnitude than “everything on Claude / everything on ChatGPT.”
...
The heatmap is not a lab score. It is the bill for software we **built, published, and operate** — most of it greenfield — through agentic delivery loops on Hermes. The same mix still evolves them. A human still sits on the merge gate; the loop does the rest.
...
| **Clinicsy** | [clinicsy.app](https://clinicsy.app) | Multi-tenant SaaS to get home care, consultórios, and clinics off spreadsheets: Maps scheduling, WhatsApp, AI clinical notes, finance. |
...
| **Profitt** | [profitt.app](https://profitt.app) | Portfolio tracker we built and operate — DeFi + traditional markets, AI insights, WhatsApp/Telegram alerts. |

These are not slides. They are public hosts: trials, alphas, paying tenants. The loops that burned 16.89B tokens are the loops that open PRs, run CI, and deploy into this catalog.

### Greenfield needed a factory, not just a good loop

A large share of that catalog started empty — a spec, a Swarm, no legacy team. That is a good way to **test models**. It is a bad way to replay the *same* workflow on the next repo, and a worse way to drop agents onto a product that already has customers.

So besides the model mix, we built **[git-meta-harness](https://github.com/brenonaraujo/git-meta-harness)** (`gmh`): a framework that materializes the delivery team — personas, GitHub issues/PRs, sensors — so Hermes and other code agents run the same loop instead of reinventing it in chat. Greenfield gets `gmh install`. A live SaaS like Clinicsy gets `gmh adopt`. We wrote that factory up in [git-meta-harness — From Loop Engineering to a Team That Ships](/blog/git-meta-harness). This post is the token bill. That post is how the work repeats.
...
16.89B on MiniMax in two months. Grok Heavy on reasoning, Build, and Experts. Gemini, OpenRouter, and local for the rest. Hermes in the middle, git-meta-harness as the factory that replays the loop. Result: **BRNN AI, Clinicsy, Mentoring, TibiaPixel**, and the rest of the catalog — real agentic operations for **~US$150/month on the current regime**, while the **same volume on metered Sonnet/Opus/GPT** is measured in **tens to hundreds of thousands of dollars**.
...
- **[BRNN AI](https://ai.brenon.cloud)**, **[Clinicsy](https://clinicsy.app)**, **[DevDojo Mentoring](https://mentoria.devdojo.academy)**, **[TibiaPixel](https://tibiapixel.brenon.cloud)**, **[OficinaCloud](https://oficina.brenon.cloud)**, **[VServer](https://vserver.brenon.cloud)**, **[Profitt](https://profitt.app)**: products in the catalog those tokens shipped.
